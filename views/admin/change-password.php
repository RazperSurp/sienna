...
if ($model->userId) {
    ...
    $user = \app\models\user\User::findOne($model->userId);
    // Предполагаем, что в модели User есть свойство fullname или username
    $targetName = $user ? ($user->fullname ?? $user->username) : ''; 
}
?>
...
<?php $form = ActiveForm::begin(); ?>
    ...
    <?= $form->field($model, 'userId')->widget(Select2::classname(), [
        'initValueText' => $targetName, // Выводим имя найденного пользователя
        'options' => ['placeholder' => 'Поиск и выбор пользователя'],
        'pluginOptions' => [
            'allowClear' => true,
